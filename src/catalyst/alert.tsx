...
export function Alert({ size = 'md', className, children, ...props }: { size?: keyof typeof sizes; className?: string; children: ReactNode } & Omit<Headless.DialogProps, 'as' | 'className'>) {
  return (
...
export function AlertTitle({ className, ...props }: { className?: string } & Omit<Headless.DialogTitleProps, 'as' | 'className'>) {
  return <Headless.DialogTitle {...props} className={clsx(className, 'text-balance text-center font-semibold text-base/6 text-zinc-950 sm:text-wrap sm:text-left sm:text-sm/6 dark:text-white')} />;
}
...
export function AlertBody({ className, ...props }: ComponentPropsWithoutRef<'div'>) {
