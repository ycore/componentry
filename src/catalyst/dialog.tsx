...
export function Dialog({ size = 'lg', className, children, ...props }: { size?: keyof typeof sizes; className?: string; children: ReactNode } & Omit<Headless.DialogProps, 'as' | 'className'>) {
  return (
...
export function DialogTitle({ className, ...props }: { className?: string } & Omit<Headless.DialogTitleProps, 'as' | 'className'>) {
  return <Headless.DialogTitle {...props} className={clsx(className, 'text-balance font-semibold text-lg/6 text-zinc-950 sm:text-base/6 dark:text-white')} />;
}
