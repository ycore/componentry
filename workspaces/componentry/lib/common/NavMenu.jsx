...
export function NavMenu({ className, navConfig }) {
    return (navConfig.sections && (<StackedLayout navbar={<Navbar className={className}>
                ...
                {section.items.map((item, itemIndex) => {
                    if (item.items) {
                        ...
                        <Dropdown.Menu key={`dropdown-menu-${itemIndex}`} anchor="bottom start" className="min-w-80 lg:min-w-64">
                          {item.items.map((subItem, subIndex) => (<Dropdown.Item key={`dropitem-${subIndex}-${subItem.url}`} href={subItem.url}>
                              {subItem.tag}
                    ...
                    return (<Navbar.Item key={`navbar-${itemIndex}-${item.url}`} href={item.url}>
                      {item.tag}
                        ...
                        if ('items' in item && item.items) {
                            ...
                            <Sidebar.Section key={`sidebar-section2-${index}-${item.url}`}>
                              {item.items.map((item, index) => {
                                    return (<Sidebar.Item key={`sidebar-${index}-${item.url}`} href={item.url}>
                                    {item.tag}
