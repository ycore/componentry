...
export function DropdownHeading({ className, ...props }) {
    return <Headless.MenuHeading {...props} className={clsx(className, 'col-span-full grid grid-cols-[1fr_auto] gap-x-12 px-3.5 pt-2 pb-1 font-medium text-sm/5 text-zinc-500 sm:px-3 sm:text-xs/5 dark:text-zinc-400')}/>;
}
...
export function DropdownShortcut({ keys, className, ...props }) {
    return (<Headless.Description as="kbd" {...props} className={clsx(className, 'col-start-5 row-start-1 flex justify-self-end')}>
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (<kbd 
        // biome-ignore lint/suspicious/noArrayIndexKey:
        key={index} className={clsx([
