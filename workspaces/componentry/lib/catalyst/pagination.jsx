...
export function PaginationPage({ href, className, current = false, children }) {
    return (<Button href={href} plain aria-label={`Page ${children}`} aria-current={current ? 'page' : undefined} className={clsx(className, 'before:-inset-px min-w-[2.25rem] before:absolute before:rounded-lg', current && 'before:bg-zinc-950/5 dark:before:bg-white/10')}>
      <span className="-mx-0.5">{children}</span>
    </Button>);
}
export function PaginationGap({ className, children = <>&hellip;</>, ...props }) {
    return (<span aria-hidden="true" {...props} className={clsx(className, 'w-[2.25rem] select-none text-center font-semibold text-sm/6 text-zinc-950 dark:text-white')}>
      {children}
    </span>);
}
